...
class ConfirmInteractiveDialog extends BaseDialog
{
    ...
    private var _sbMessage:Scrollbar;
    ...
    override public function init(skin:DialogSkin):Void
    {
        ...
        _txtMessage.addEventListener(TextEvent.LINK, _textEventLinkHandler);
		...
		if ( _icon != null )
			addChild( _icon );
		/*
		_sbMessage = new Scrollbar();
		_sbMessage.options.gripStyle = GripStyle.LINE;
		_sbMessage.options.gripColor = Colors.GREY2;
		_sbMessage.options.background = false;
        _sbMessage.target = _txtMessage;
		addChild( _sbMessage );
		*/
    ...
    override public function draw( parentSize:Point ) : Void
    {
		...
		//var messageMaxHeight:Int = 220;
		var dialogWidth:Float = _parentSize.x * 0.30;
		var dialogHeight:Float = 0;	
		...
		/*if ( _txtMessage.height > messageMaxHeight )
		{
			_txtMessage.autoSize = TextFieldAutoSize.NONE;
			_txtMessage.height = messageMaxHeight;
		}*/
		...
		/*
		_sbMessage.x = _txtMessage.x + _txtMessage.width;
		_sbMessage.y = _txtMessage.y;
		_sbMessage.update();	
        _sbMessage.visible = (_txtMessage.maxScrollV > 1);*/
