...
/**
	 ...
	 */
class DuplicateDOPlanningPopup extends Sprite
{
    private var _doScrollList:ScrollList;
    ...
    public var doListBackground:Sprite;
    ...
    private function init(e:Event):Void
    {
        ...
        background.height = txtMessage.height + 40 + btValid.height + 200;
    ...
    private function _displayDOList():Void
    {
        if (_doScrollList != null)
        {
            removeChild(_doScrollList);
        }
        
        _doScrollList = new ScrollList(DO.list, 400, 200, "labelFullForButton", ScrollItemDO);
        _doScrollList.x = background.x + (background.width - 400) / 2;
        _doScrollList.y = txtMessage.y + txtMessage.height + 10;
    ...
    private function clickValidHandler(e:MouseEvent):Void
    {
        if (_selectedDO != null)
        {
            //PopupManager.instance.closePopup(_selectedDO);
